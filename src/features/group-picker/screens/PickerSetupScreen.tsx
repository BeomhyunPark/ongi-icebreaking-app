...
export function PickerSetupScreen({ mode, names, nameDraft, outcomes, outcomeDraft, winnerCount, groupCount, error, selectedMode, selectMode, clearItems, setNames, setNameDraft, setOutcomes, setOutcomeDraft, setWinnerCount, setGroupCount, prepareDraw, onBackHome }: Pick<PickerController, 'mode' | 'names' | 'nameDraft' | 'outcomes' | 'outcomeDraft' | 'winnerCount' | 'groupCount' | 'error' | 'selectedMode' | 'selectMode' | 'clearItems' | 'setNames' | 'setNameDraft' | 'setOutcomes' | 'setOutcomeDraft' | 'setWinnerCount' | 'setGroupCount' | 'prepareDraw'> & { onBackHome: () => void }) {
  return (
      ...
      <section className="group-picker-section" aria-labelledby="picker-names-title">
        <div className="group-picker-section__heading">
          ...
          <div className="group-picker-section__controls">
            ...
            {names.length > 0 ? (
              <button type="button" onClick={clearItems}>모두 지우기</button>
            ) : null}
          </div>
        </div>
        <ItemEditor
