...
type DrawPhase = { phase: 'setup'; result: null } | { phase: 'drawing' | 'result'; result: DrawResult };
...
const emptyReveal = () => ({ activeLadderStart: null, revealedLadderStarts: new Set<number>(), revealAllQueue: null });

export function createPickerState(mode: PickerMode, setup: PickerSetup, result: DrawResult | null = null, revealed: number[] = []): PickerState {
  return { mode, setup, error: '', ...emptyReveal(), revealedLadderStarts: new Set(revealed),
...
export function pickerReducer(state: PickerState, action: PickerAction): PickerState {
  switch (action.type) {
    ...
    case 'EDIT_NAMES':
      return { ...state, error: '', setup: { ...state.setup, names: action.names, outcomes: state.setup.outcomes.slice(0, action.names.length) } };
    case 'EDIT_DRAFT':
    ...
    case 'CLEAR_ITEMS':
      return { ...state, setup: { ...state.setup, names: [], outcomes: [], nameDraft: '', outcomeDraft: '' } };
    case 'PREPARE_DRAW':
      return action.prepared.ok
        ? { ...state, ...emptyReveal(), setup: action.prepared.setup, result: action.prepared.result, phase: 'drawing', error: '' }
    ...
    case 'REVEAL_START':
      if (state.phase !== 'result' || state.result.mode !== 'ladder' || state.activeLadderStart !== null
        || action.index < 0 || action.index >= state.result.orderedNames.length) return state;
      return { ...state, activeLadderStart: action.index };
    case 'REVEAL_ALL': {
      if (state.phase !== 'result' || state.result.mode !== 'ladder' || state.activeLadderStart !== null) return state;
      const { result } = state;
      const special = getSpecialOutcomeValues(result.outcomes);
      const queue = resolveLadder(result.ladder).flatMap((destination, start) => (
        special.has(result.outcomes[destination]) && !state.revealedLadderStarts.has(start) ? [start] : []
      ));
      return queue.length
    ...
    case 'TRACE_FINISHED': {
      ...
      return { ...state, activeLadderStart: remaining[0] ?? null, revealAllQueue: remaining.length ? remaining : null,
        revealedLadderStarts: remaining.length ? revealed : new Set(state.result.orderedNames.map((_, index) => index)) };
    }
  }
}
