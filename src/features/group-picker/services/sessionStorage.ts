...
function parseLadder(value: unknown, columnCount: number): Ladder | null {
  if (
    ...
    !isIntegerBetween(value.rowCount, 1, 32) ||
  ...
  for (const rung of value.rungs) {
    if (
      !isRecord(rung) ||
      !isIntegerBetween(rung.row, 0, rowCount - 1) ||
      !isIntegerBetween(rung.leftColumn, 0, columnCount - 2)
    )
      return null;
    const { row, leftColumn } = rung;
    if (
      rungs.some(
        (existing) => existing.row === row && Math.abs(existing.leftColumn - leftColumn) <= 1,
      )
    )
      return null;
...
export function parseDrawResult(value: unknown, mode: PickerMode): DrawResult | null {
  if (
    !isRecord(value) ||
    value.mode !== mode ||
    !isLabels(value.orderedNames) ||
    value.orderedNames.length < 2 ||
    new Set(value.orderedNames).size !== value.orderedNames.length
  )
    return null;
  ...
  switch (mode) {
    case 'ladder': {
      ...
      return ladder && isLabels(value.outcomes) && value.outcomes.length === orderedNames.length
        ? { mode, orderedNames, ladder, outcomes: value.outcomes }
        : null;
    }
    case 'lottery':
      return isIntegerBetween(value.winnerCount, 1, orderedNames.length - 1)
        ? { mode, orderedNames, winnerCount: value.winnerCount }
        : null;
    case 'groups':
    case 'pairs':
      return Array.isArray(value.groups) &&
        value.groups.every(isLabels) &&
        samePeople(value.groups.flat(), orderedNames)
        ? { mode, orderedNames, groups: value.groups }
        : null;
    case 'supporter': {
      ...
      for (const assignment of value.supportAssignments) {
        if (
          !isRecord(assignment) ||
          typeof assignment.supporter !== 'string' ||
          typeof assignment.recipient !== 'string' ||
          assignment.supporter === assignment.recipient
        )
          return null;
      ...
      return samePeople(
        supportAssignments.map((item) => item.supporter),
        orderedNames,
      ) &&
        samePeople(
          supportAssignments.map((item) => item.recipient),
          orderedNames,
        )
        ? { mode, orderedNames, supportAssignments }
        : null;
    }
    default:
...
export function parsePickerSession(value: unknown, mode: PickerMode): PickerSession | null {
  if (
    !isRecord(value) ||
    !isLabels(value.names) ||
    new Set(value.names).size !== value.names.length ||
    !isLabels(value.outcomes) ||
    typeof value.nameDraft !== 'string' ||
    typeof value.outcomeDraft !== 'string' ||
    !isIntegerBetween(value.winnerCount, 1, 32) ||
    !isIntegerBetween(value.groupCount, 2, 32)
  )
    return null;
  ...
  if (
    ...
    !value.revealed.every((index) =>
      isIntegerBetween(index, 0, (result?.orderedNames.length ?? 0) - 1),
    )
  )
    return null;
  return {
    names: value.names,
    nameDraft: value.nameDraft,
    outcomes: value.outcomes,
    outcomeDraft: value.outcomeDraft,
...
export function savePickerSession(mode: PickerMode, session: PickerSession): void {
  // Keep the v1 wire format readable by v2.3.0 as well, including on rollback.
  const result = session.result
    ? {
        winnerCount: Math.min(session.winnerCount, session.result.orderedNames.length - 1),
        ladder: null,
        outcomes: [],
        groups: [],
        supportAssignments: [],
    ...
    : null;
  writeStoredValue(key(mode), { ...session, result });
}
